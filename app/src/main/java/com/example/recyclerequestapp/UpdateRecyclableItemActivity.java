...
public class UpdateRecyclableItemActivity extends AppCompatActivity {
    ...
    private RecyclableItem existingItem;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        btnUpdateItem = findViewById(R.id.btn_save_item);
        tvFormTitle = findViewById(R.id.tv_form_title);
        tvFormTitle.setText("Update Recyclable Item");
        ...
        if (getIntent().hasExtra("item_object")) {
            // Get as Serializable and cast
            existingItem = (RecyclableItem) getIntent().getSerializableExtra("item_object");
            if (existingItem != null) {
                populateForm(existingItem);
            } else {
            ...
            Toast.makeText(this, "Error: No item selected for update.", Toast.LENGTH_SHORT).show();
            finish();
            return;
        }
    ...
    private void populateForm(RecyclableItem item) {
        etItemName.setText(item.getItemName());
        etPricePerKg.setText(String.valueOf(item.getPricePerKg()));
    }
    ...
    private void updateItem() {
        ...
        RecyclableItem updatedItem = new RecyclableItem(existingItem.getId(), itemName, pricePerKg);

        Log.d("API_CALL", "Updating item with token: " + authToken + ", ID: " + updatedItem.getId());

        itemService.updateRecyclableItem(updatedItem.getId(), updatedItem).enqueue(new Callback<RecyclableItem>() {
            @Override
            public void onResponse(@NonNull Call<RecyclableItem> call, @NonNull Response<RecyclableItem> response) {
                if (response.isSuccessful()) {
                    ...
                    handleApiError(response, "update item");
                }
            }

            @Override
            public void onFailure(@NonNull Call<RecyclableItem> call, @NonNull Throwable t) {
                Toast.makeText(UpdateRecyclableItemActivity.this, "Network error: " + t.getMessage(), Toast.LENGTH_SHORT).show();
                Log.e("UpdateRecyclableItem", "Update item failure", t);
            }
        });
    }
    ...
    private void handleApiError(Response<?> response, String operation) {
        ...
        try {
            ...
            Log.e("UpdateRecyclableItem", "Error " + operation + ": Code " + response.code() + ", Message: " + response.message() + ", Body: " + errorBody);
            ...
            Log.e("UpdateRecyclableItem", "Error reading errorBody", e);
